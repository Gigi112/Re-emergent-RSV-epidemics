covid_contact_model <- function(t,y,parms, time.step='month'){
  ...
  if(t>465 & t<=468){
    c_matrix=c2hs_new # different contact patterns during the stay-at-home order
  }else{
    c_matrix=c2} # other than the three month stay-at-home order, we assumed contact patterns stay the same
 ...
 ###Check the standardization of beta and overall structure of lambda here
  ...
  b <- baseline.txn.rate/ (parms$dur.days1/length.step) # transmission probability per unit time
  beta <-  (b/100)/(sum(yinit.matrix)^(1-q))*c_matrix# q depends on transmission type (whether depends on population density or not); c2 is the contact matrix
