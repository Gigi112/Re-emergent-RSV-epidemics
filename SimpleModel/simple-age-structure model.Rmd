...
```{r}
plot(1:408,rowSums(H1),type="l",xlab="months",ylab="simulated RSV hospitalizations")
# need to get rid of the burn-in period

barplot(colSums(H1),xlab="age group",ylab="Number of Hospitalizations") # simulated age distribution Age groups: The <12 month olds, were divided into monthy age classes. The 1-4 year olds are  divided into 1yearly age classes, then 5–9 years, 10–19 years, 20–39 years, 40–59-years, and 60+ years old

Hosp_sim <- as.integer(rowSums(H1[301:408,]))
agedist_Sim <- colSums(H1[301:408,])
```
...
fitmodel <-  function(parameters,dat) {   # takes the parameter values and dataset as inputs
  ...
  H <- rowSums(H1)
   ...
   LLmulti <- dmultinom(x= colSums(H1),prob = agedist_Sim,log = T) # fit to age distribution
